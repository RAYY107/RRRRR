--[[
    Evora ID — adapter: vRP 1.x / vRPex (plain-argument proxy)
...
local Adapter = { name = 'vrp_modern', keyPrefix = 'vrp' }
EvoraAdapters.vrp_modern = Adapter

local names = {}

function Adapter:detect()
    return EvoraVrp.running() and EvoraVrp.hasFile('lib/Proxy.lua') and not EvoraVrp.usesTableArgs()
end
...
function Adapter:getUserId(src)
    local id = self.vRP.getUserId(src)
...
function Adapter:hasPermission(src, perm)
    ...
    return self.vRP.hasPermission(id, perm) == true
end
...
function Adapter:getName(src)
    ...
    local ok, identity = pcall(self.vRP.getUserIdentity, id)
